"""Command-line verification helpers for the Phase 2 deterministic core."""
...
def _email(data_root: Path, email_id: str) -> int:
    record_path = data_root / "inbox" / f"{email_id}.json"
    if not record_path.exists():
        raise SystemExit(f"Email not found: {record_path}")
    record = json.loads(record_path.read_text(encoding="utf-8"))
    print(json.dumps(process_email_record(record, data_root), indent=2, ensure_ascii=False))
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(description="CargoClarity Phase 2 deterministic core")
    ...
    return _email(args.data_root, args.email_id)
